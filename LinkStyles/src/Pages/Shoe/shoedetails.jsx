...
function ShoeDetails() {
  ...
  useEffect(() => {
    if (product?.category) {
      fetch(
        `${URL}/api/v1/shoes?name=${encodeURIComponent(product.productName)}`,
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  const shoeImages = [product?.image1, product?.image2, product?.image3];
          ...
          <div className={styles.productDetailSections}>
            ...
            <div className={styles.detailsSection}>
              <div className={styles.detailsubsection}>
                ...
                <small
                  style={
                    stockCount <= 20 ? { color: "red" } : { color: "green" }
                  }
                >
                  {" "}
